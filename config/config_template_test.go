...
func TestTemplateParsesAsYAML(t *testing.T) {
	...
	assert.Empty(t, cfg.Block.Packages)
	assert.Empty(t, cfg.Block.Message)
}
...
func TestTemplateMatchesDefaults(t *testing.T) {
	...
	assert.Equal(t, def.Block.Message, parsed.Block.Message, "block.message mismatch")
	assert.Equal(t, len(def.Block.Packages), len(parsed.Block.Packages), "block.packages mismatch")

...
// GitDirPath, GitConfigPath and GitHooksPath are the git-specific mandatory
// deny targets, relative to a repository root ($CWD or $HOME). The whole
// .git directory is write-denied unless allow_git_config is set: hooks and
// config also live under .git/modules/<name> for submodules and under
// .git/worktrees/<name>, and git runs them outside the sandbox. A tool that
// must write there names ${CWD}/.git in allow_write (see the git preset).
const (
	GitDirPath    = ".git"
...
//
// The list is not exhaustive. Coding-agent hook directories (.claude,
// .cursor, .gemini, .mcp.json), .devcontainer, .idea and the git-hook
// manager files are candidates that need their own friction review. .husky
// is regenerated by husky's prepare script on every install, so a directory
// deny there breaks a common flow. .envrc is out: direnv refuses to run a
// changed file until the user allows it again.

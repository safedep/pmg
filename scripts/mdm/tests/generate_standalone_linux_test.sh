...
INSTALL_ONE="${OUT_ONE}/pmg_setup_install_linux_standalone.sh"
UNINSTALL_ONE="${OUT_ONE}/pmg_uninstall_linux_standalone.sh"
INSTALL_TWO="${OUT_TWO}/pmg_setup_install_linux_standalone.sh"
UNINSTALL_TWO="${OUT_TWO}/pmg_uninstall_linux_standalone.sh"
...
for output in "$INSTALL_ONE" "$UNINSTALL_ONE" "$INSTALL_TWO" "$UNINSTALL_TWO"; do
  ...
  assert_not_contains "$output" "source \"\${SCRIPT_DIR}/lib_linux.sh\""
  assert_contains "$output" 'require_linux()'
  assert_contains "$output" 'each_target_user()'
  assert_contains "$output" 'run_user_session()'
  assert_contains "$output" '# Code generated by generate_standalone_linux.sh. Do not edit directly.'
done
...
mkdir -p "${DIRECTORY_TARGET_OUT}/pmg_setup_install_linux_standalone.sh"
...
mkdir -p "${SOURCE_OVERWRITE_FIXTURE}/lib" "${SOURCE_OVERWRITE_FIXTURE}/linux"
cp "$GENERATOR" \
  "$SOURCE_OVERWRITE_FIXTURE"
cp "${SCRIPT_DIR}/lib/generate_standalone_lib.sh" \
  "${SOURCE_OVERWRITE_FIXTURE}/lib/"
cp "${SCRIPT_DIR}/linux/lib_linux.sh" \
  "${SCRIPT_DIR}/linux/pmg_setup_install_linux.sh" \
  "${SCRIPT_DIR}/linux/pmg_uninstall_linux.sh" \
  "${SOURCE_OVERWRITE_FIXTURE}/linux/"
"${SOURCE_OVERWRITE_FIXTURE}/generate_standalone_linux.sh" \
  --output-dir "$SOURCE_OVERWRITE_FIXTURE"
cmp "${SCRIPT_DIR}/linux/pmg_setup_install_linux.sh" \
  "${SOURCE_OVERWRITE_FIXTURE}/linux/pmg_setup_install_linux.sh"
cmp "${SCRIPT_DIR}/linux/pmg_uninstall_linux.sh" \
  "${SOURCE_OVERWRITE_FIXTURE}/linux/pmg_uninstall_linux.sh"
assert_file "${SOURCE_OVERWRITE_FIXTURE}/pmg_setup_install_linux_standalone.sh"
assert_file "${SOURCE_OVERWRITE_FIXTURE}/pmg_uninstall_linux_standalone.sh"
...
CONFIG_INSTALL="${CONFIG_OUT}/pmg_setup_install_linux_standalone.sh"
CONFIG_UNINSTALL="${CONFIG_OUT}/pmg_uninstall_linux_standalone.sh"
...
mkdir -p "${MARKER_FIXTURE}/lib" "${MARKER_FIXTURE}/linux"
cp "$GENERATOR" \
  "$MARKER_FIXTURE"
cp "${SCRIPT_DIR}/lib/generate_standalone_lib.sh" \
  "${MARKER_FIXTURE}/lib/"
cp "${SCRIPT_DIR}/linux/lib_linux.sh" \
  "${SCRIPT_DIR}/linux/pmg_setup_install_linux.sh" \
  "${SCRIPT_DIR}/linux/pmg_uninstall_linux.sh" \
  "${MARKER_FIXTURE}/linux/"
awk '
  NR > 1 && /^#/ && $0 != "# shellcheck source=lib_linux.sh" {
    printf "# Fixture installer prose %d\n", NR
    next
  }
  { print }
' "${SCRIPT_DIR}/linux/pmg_setup_install_linux.sh" \
  > "${MARKER_FIXTURE}/linux/pmg_setup_install_linux.sh"
if cmp -s \
  "${SCRIPT_DIR}/linux/pmg_setup_install_linux.sh" \
  "${MARKER_FIXTURE}/pmg_setup_install_linux.sh"; then
  fail "marker-independence fixture did not change installer prose"
fi
"${MARKER_FIXTURE}/generate_standalone_linux.sh" \
  --config "$CONFIG" \
  --output-dir "${MARKER_FIXTURE}/output"
assert_contains \
  "${MARKER_FIXTURE}/output/pmg_setup_install_linux_standalone.sh" \
...
assert_absent "${OVERSIZED_OUTPUT}/pmg_setup_install_linux_standalone.sh"
assert_absent "${OVERSIZED_OUTPUT}/pmg_uninstall_linux_standalone.sh"

OVERSIZED_EXISTING_OUTPUT="${TEST_ROOT}/oversized-intune-existing-output"
mkdir -p "$OVERSIZED_EXISTING_OUTPUT"
printf '%s\n' "existing installer" \
  > "${OVERSIZED_EXISTING_OUTPUT}/pmg_setup_install_linux_standalone.sh"
printf '%s\n' "existing uninstaller" \
  > "${OVERSIZED_EXISTING_OUTPUT}/pmg_uninstall_linux_standalone.sh"
assert_fails "$GENERATOR" \
  --config "$OVERSIZED_CONFIG" \
  --output-dir "$OVERSIZED_EXISTING_OUTPUT"
assert_equals "existing installer" \
  "$(cat "${OVERSIZED_EXISTING_OUTPUT}/pmg_setup_install_linux_standalone.sh")" \
  "oversized generation must not replace installer"
assert_equals "existing uninstaller" \
  "$(cat "${OVERSIZED_EXISTING_OUTPUT}/pmg_uninstall_linux_standalone.sh")" \
...
AMBIENT_INSTALL="${AMBIENT_OUT}/pmg_setup_install_linux_standalone.sh"
AMBIENT_UNINSTALL="${AMBIENT_OUT}/pmg_uninstall_linux_standalone.sh"
...
CREDENTIAL_INSTALL="${CREDENTIAL_OUT}/pmg_setup_install_linux_standalone.sh"
CREDENTIAL_UNINSTALL="${CREDENTIAL_OUT}/pmg_uninstall_linux_standalone.sh"
...
mkdir -p \
  "${FAILED_UNINSTALL_OUT}/pmg_uninstall_linux_standalone.sh"
printf '%s\n' "existing installer" \
  > "${FAILED_UNINSTALL_OUT}/pmg_setup_install_linux_standalone.sh"
...
assert_equals "existing installer" \
  "$(cat "${FAILED_UNINSTALL_OUT}/pmg_setup_install_linux_standalone.sh")" \
  "failed uninstaller generation must not rewrite installer"

FAILED_INSTALL_OUT="${TEST_ROOT}/failed-installer"
mkdir -p "${FAILED_INSTALL_OUT}/pmg_setup_install_linux_standalone.sh"
printf '%s\n' "existing uninstaller" \
  > "${FAILED_INSTALL_OUT}/pmg_uninstall_linux_standalone.sh"
assert_fails env SAFEDEP_API_KEY="$TEST_API_KEY" \
  SAFEDEP_TENANT_ID="$TEST_TENANT_ID" \
  "$GENERATOR" \
  --embed-cloud-credentials \
...
assert_equals "existing uninstaller" \
  "$(cat "${FAILED_INSTALL_OUT}/pmg_uninstall_linux_standalone.sh")" \
...
assert_variant "credentials-only" \
  "${CHILD_PROBE_OUT}/pmg_setup_install_linux_standalone.sh" \
  "${CHILD_PROBE_OUT}/pmg_uninstall_linux_standalone.sh" \
  "no" "yes" "700"
assert_variant "config-and-credentials" \
  "$CREDENTIAL_INSTALL" "$CREDENTIAL_UNINSTALL" "yes" "yes" "700"

for generic_output in \
  "${SCRIPT_DIR}/standalone/pmg_setup_install_linux_standalone.sh" \
  "${SCRIPT_DIR}/standalone/pmg_uninstall_linux_standalone.sh"; do

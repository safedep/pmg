#!/bin/bash
# Run the multi-file MDM scripts end to end on a disposable Linux CI runner.
...
if [[ "${CI:-}" != "true" || "${PMG_MDM_E2E:-}" != "1" ]]; then
  echo "Error: multifile Linux E2E requires CI=true and PMG_MDM_E2E=1" >&2
  exit 1
fi
...
E2E_USER="pmge2e"
E2E_USER_HOME="/home/${E2E_USER}"
E2E_USER_CONFIG="${E2E_USER_HOME}/.config/safedep/pmg/config.yml"

create_pmg_wrapper

cleanup() {
  set +e
  sudo -n userdel -r "$E2E_USER" >/dev/null 2>&1
...
EOF
  ...
  sudo -n /usr/bin/env \
    ...
    "${STAGE_DIR}/pmg_setup_install_linux.sh"
...
test_multifile_multi_user_install() {
  ...
  reset_wrapper_captures
  if ! add_output=$(sudo -n useradd -m -s /bin/bash "$E2E_USER" 2>&1) ||
    ! id "$E2E_USER" >/dev/null 2>&1; then
    printf '%s\n' "$add_output"
    fail "useradd did not create the test user"
  fi
  sudo -n test -d "$E2E_USER_HOME" ||
    fail "useradd did not create the test user home"
  ...
  if ! del_output=$(sudo -n userdel -r "$E2E_USER" 2>&1) ||
    id "$E2E_USER" >/dev/null 2>&1; then
    printf '%s\n' "$del_output"
    fail "userdel did not delete the test user"
  fi
}

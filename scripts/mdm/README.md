...
On Windows, the script runs `pmg setup remove --system` first. Steps 1 and 3 then run for logged-on users only, and for every local profile the script deletes `%APPDATA%\safedep\pmg`, `%LOCALAPPDATA%\safedep\pmg` and `%USERPROFILE%\.pmg`. Last, it deletes `pmg.exe` and both of its machine `PATH` entries, and removes the managed config.

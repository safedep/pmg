...
func DefaultPipxPackageManagerConfig() PypiPackageManagerConfig {
	return PypiPackageManagerConfig{
		InstallCommands: []string{"install", "inject"},
		// We omit `run` from both lists because `pipx run` downloads and executes in a single command,
		// and requires special parsing logic to identify the package target.
		NonDownloadCommands: []string{
...
func NewPypiPackageManager(config PypiPackageManagerConfig) (*pypiPackageManager, error) {
	...
	case "pip3":
		parser = NewPipCommandParser(config)
	case "pipx":
		parser = NewPipxCommandParser(config)
	default:
...
type pipxCommandParser struct {
	config PypiPackageManagerConfig
}

func NewPipxCommandParser(config PypiPackageManagerConfig) pypiCommandParser {
	return &pipxCommandParser{
		config: config,
	}
}

func (p *pipxCommandParser) ParseCommand(args []string) (*ParsedCommand, error) {

...
For MDM fleets, also schedule `pmg cloud sync` hourly (launchd, systemd timer, or your MDM's script scheduler). Each sync also sends a check-in behind an hourly cooldown, so an idle endpoint still appears as active in the SafeDep Cloud Endpoints view. No `pmg setup doctor` run is needed for the endpoint to appear. Hourly is the supported cadence: the server refreshes endpoint activity at most every 30 minutes and the console marks an endpoint recent within 24 hours. See `cloud.check_in` in the config template to tune or disable it.

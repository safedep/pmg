...
Uninstall from Apps & Features, or with `msiexec /x pmg_Windows_x86_64.msi /qn`. The uninstall runs `pmg setup remove --system --config-file` before it deletes the binary. This removes the managed config too, the same as the MDM uninstall script. The uninstall deletes the shim directory even when `pmg setup remove --system` cannot run, for example when the binary is already gone, so no shim outlives the binary. Under a running `pmg.exe`, the uninstall removes the shims, the shim directory's PATH entry, the config and the Apps & Features entry, and Windows deletes the locked binary at the next restart. The PATH entry for the directory of `pmg.exe` stays, as after `pmg setup remove --system`. An upgrade under a running `pmg.exe` renames the running file to `pmg.exe.old.<product code>` and installs the new file at once. The running process keeps the old build until it restarts. The next upgrade or the uninstall deletes the old file. A backup that a process still holds at that time stays in the product directory until an administrator deletes it. In this case `msiexec` can return 3010 (restart required), because Windows Installer checks for files in use before the rename. The new file is in place and no restart is needed. An MDM that treats 3010 as a soft reboot can map it to success for this package. An edge release ships an MSI too. Apps & Features lists it as `pmg (edge)`. It replaces a stable install of the same version, and the next stable build replaces it.

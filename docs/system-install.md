...
At run time, PMG on Windows obeys the managed config only when Administrators or SYSTEM own the file and it is not a link or junction. A file at that path with another owner is ignored with a warning, so a file a standard user planted before any install cannot become the machine policy. `pmg setup doctor` reports a managed config whose descriptor differs from the one the install wrote.
